...
export default function Profile() {
  ...
  const postUserInfo = async () => {
    await axios.post(
      "https://twiki.csc.depauw.edu:5000/profile/update",
      userInfo,
      {
        headers: {
          authorization: idToken,
  ...
  const removePreference = (e) => {
    if (e.target.id === "prefYear") {
      ...
      const yearValue = numericYearCollegeYearConvert(
        ...
        <div className={styles.profile__bio}>
          ...
          <div>
            ...
            <input
              ...
              min="1"
              max="3"
              value={getPriority("prefYear")}
              onChange={onPriorityChange}
              step="1"
            />
            {userInfo?.prefYear?.map((prefYear) => {
              return (
                <div onClick={removePreference} id="prefYear">
            ...
            <input
              ...
              min="1"
              max="3"
              value={getPriority("prefMajor")}
              onChange={onPriorityChange}
              step="1"
            />
            {userInfo?.prefMajor?.map((prefMajor) => {
              return (
                <div onClick={removePreference} id="prefMajor">
                  {prefMajor}
                </div>
              );
            })}
            ...
            <input
              ...
              min="1"
              max="3"
              value={getPriority("hobby")}
              onChange={onPriorityChange}
              step="1"
            />
            {userInfo?.hobbies?.map((hobby) => {
              return (
                <div onClick={removePreference} id="hobby">
                  {hobby}
                </div>
              );
            })}
          ...
          <div className={styles.profile__update}>
            <button type="submit" onClick={postUserInfo}>
              Update My Status
            </button>
            <p>5 times left</p>
          </div>
